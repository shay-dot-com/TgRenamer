...
async def receiver(client: Client, message: Message):
    if not db:
        await message.reply_text("Database is not connected. Cannot process files.")
        return
    ...
    if file.file_size > 2000000000:
        ...
        if not userbot:
            await message.reply_text("This file is larger than 2GB, but Premium 4GB support is not configured.")
            return

    processing_msg = await message.reply_text("Added to Queue ⏳")
